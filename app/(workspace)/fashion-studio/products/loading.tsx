...
export default function FashionStudioLoading() {
  return (
    <div aria-busy="true" aria-label="Loading Product Library">
      ...
      <div className="mt-[var(--space-section)] grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {Array.from({ length: 3 }).map((_, index) => (
          <Card className="min-h-64 animate-pulse p-6" key={index}>
            <div className="size-11 rounded-card bg-surface-highlight" />
            <div className="mt-6 h-6 w-2/3 rounded-control bg-surface-highlight" />
            <div className="mt-3 h-4 w-1/2 rounded-control bg-surface-highlight" />
            <div className="mt-16 h-4 w-24 rounded-control bg-surface-highlight" />
          </Card>
        ))}
      </div>
    </div>
